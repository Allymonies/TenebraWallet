...
const debug = Debug("tenebraweb:address-picker-hints");

const HINT_LOOKUP_DEBOUNCE = 250;

interface PickerHintsRes {
  pickerHints: JSX.Element | null;
  foundAddress?: TenebraAddressWithNames | false;
  foundName?: TenebraName | false;
}
  ...
  const lookupHint = useMemo(() => debounce(async (
    ...
    if (hasAddress) {
      ...
      try {
        ...
        const res = await api.get<{ name: TenebraName }>(
